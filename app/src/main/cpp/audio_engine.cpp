    ...
    for (auto& track : tracks_) {
        ...
        for (auto& clip : track.clips) {
            ...
            for (int32_t i = 0; i < numFrames; i++) {
                ...
                const float srcL = clip.buffer.interleavedStereo[srcFrame * 2] * clipGain;
                const float srcR = clip.buffer.interleavedStereo[srcFrame * 2 + 1] * clipGain;

                outputInterleavedStereo[i * 2] += srcL * leftGain;
                outputInterleavedStereo[i * 2 + 1] += srcR * rightGain;
            }
        }
    }
